...
int main (void)
{
	...
	*/
		...
		GPIO_PUSH_BUTTON.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PULL_DOWN;
		...
		//value = GPIO_ReadFromInputPin(GPIOC,GPIO_PIN_NO_13);
	...
	*/
	...
	/*
		...
		// LEDs state persist once button pressed
	...
	// initialization for LED
		...
		while(1)
		{
			...
			if(value == GPIO_PIN_CLEAR && reference == GPIO_PIN_SET)
			{
	...
	*/
	/*
	    ...
	    while(1)
	    {
	    	...
	    	if(RST_Counter <= 250000)
	    	{
	    		flag = 1;
	    	}
	    //	else if(RST_Counter>= 500000)
	    //	{
	    //		RST_Counter = 0;
	    //	}
	    	else
	    	...
	    	if(flag)
	    	{
	    		...
	    		GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_7, GPIO_PIN_CLEAR);
	    	}
	    	else
	    	{
	    		GPIO_WriteToOutputPin(GPIOA, GPIO_PIN_NO_6, GPIO_PIN_CLEAR);
